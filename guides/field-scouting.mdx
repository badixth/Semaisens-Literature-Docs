...
**Overdue is a flag, not a state.** `task.overdue` is an **event** in [activity bindings](/concepts/risk-model#activity-bindings) that fires `promote_severity`. A task past its window is still `assigned` or `accepted` — it is late, not elsewhere. Nothing transitions _into_ overdue and nothing transitions out of it by action; only the clock and the work decide it.
