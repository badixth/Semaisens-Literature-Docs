...
Who may acknowledge: an in-scope `agronomist` who is not the author. Fallback: an in-scope `estate_manager` where no agronomist has been active for 24 hours. The fallback is recorded on `acknowledged_by_role` and disclosed wherever the acknowledgement is shown; a manager's acknowledgement is not an agronomist's and the record must not imply otherwise.
...
- An acknowledged finding displays `acknowledged_by` (identity and role) and, when the fallback rule was used, that the acknowledgement was recorded by an `estate_manager` because no in-scope `agronomist` was active in the preceding 24 hours.
...
| `fnd_seed_palm_ganoderma_ack_fallback_05b` | Oil palm field 3 | Oil palm | human, `agronomist`, `critical` (acknowledged by `estate_manager`, fallback path) | Estate has no in-scope `agronomist` with activity in the preceding 24 hours; the in-scope `estate_manager` records the acknowledgement. Exercises the fallback rule per [Guardrails → Preconditions](#guardrails), the `acknowledged_by_role = estate_manager` audit, and the disclosure that a manager's acknowledgement, not an agronomist's, is on the record. |
...
- Acknowledgement on the normal path fires via fixture 05 (in-scope `agronomist`); the fallback fires via fixture 05b (in-scope `estate_manager`, no agronomist active in 24 hours).
...
| **AC-15** | [Role Model](/snippets/role-model) does not name the acknowledgement fallback: in-scope `estate_manager` when no in-scope `agronomist` has activity in the preceding 24 hours. | The role-model page should record the fallback so the entitlement rules for acknowledgement are complete in one place. Amendment C's [Guardrails → Preconditions](#guardrails) row is the current source. |
