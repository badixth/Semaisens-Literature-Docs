...
Who may acknowledge: an in-scope `agronomist` who is not the author. Fallback: an in-scope `estate_manager` where no other in-scope `agronomist` has been active for 24 hours. The author's own activity does not count toward the test, since the author cannot acknowledge their own finding; on a single-agronomist estate the fallback is the only path. The fallback is recorded on `acknowledged_by_role` and disclosed wherever the acknowledgement is shown; a manager's acknowledgement is not an agronomist's and the record must not imply otherwise.
...
<Note>
  ...
  **Scout ceiling is `medium`.** A scout who believes what they are seeing warrants `high` or `critical` raises the finding at their ceiling and requests promotion; an in-scope `agronomist` promotes in one action and the promoted finding is authored at the higher band by that agronomist. The scout's original observation and the promoting agronomist are both recorded on the audited finding.
...
- An acknowledged finding displays `acknowledged_by` (identity and role) and, when the fallback rule was used, that the acknowledgement was recorded by an `estate_manager` because no in-scope `agronomist` other than the author was active in the preceding 24 hours.
...
| `fnd_seed_palm_ganoderma_ack_fallback_05b` | Oil palm field 3 | Oil palm | human, `agronomist`, `critical` (acknowledged by `estate_manager`, fallback path, idle peer) | Multi-agronomist estate whose other in-scope `agronomist`s have all been idle for more than 24 hours; the in-scope `estate_manager` records the acknowledgement. Exercises the fallback rule per [Guardrails → Preconditions](#guardrails) on the idle-peers path, the `acknowledged_by_role = estate_manager` audit, and the disclosure that a manager's acknowledgement, not an agronomist's, is on the record. |
| `fnd_seed_palm_ganoderma_ack_fallback_solo_12` | Oil palm field 4 | Oil palm | human, `agronomist`, `critical` (acknowledged by `estate_manager`, fallback path, single-agronomist estate) | Estate has only one in-scope `agronomist` and that agronomist is the `author_ref`; the author's own activity is excluded from the fallback test, so the in-scope `estate_manager` records the acknowledgement. Exercises the author-exclusion clause of the fallback rule per [Guardrails → Preconditions](#guardrails): single-agronomist estates always take the fallback path when their agronomist raises. |
...
- Acknowledgement on the normal path fires via fixture 05 (in-scope `agronomist`); the fallback fires via fixture 05b (in-scope `estate_manager`, all other in-scope agronomists idle for 24 hours) and via fixture 12 (single-agronomist estate whose only agronomist is the author, so the author-exclusion clause forces the fallback path).
...
| **AC-15** | [Role Model](/snippets/role-model) does not name the acknowledgement fallback: in-scope `estate_manager` when no in-scope `agronomist` other than the `author_ref` has activity in the preceding 24 hours. | The role-model page should record the fallback so the entitlement rules for acknowledgement are complete in one place, including that the author's own activity is excluded from the test (single-agronomist estates always take the fallback). Amendment C's [Guardrails → Preconditions](#guardrails) row is the current source. |
