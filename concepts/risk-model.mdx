...
**Fail-closed rule** (repeated here so it is not lost inside the section): a crop-product pair with no `sourced` band produces a refusal, not a silent pass. This applies to VRA validation, Semai Advisor writes, and the failure-modes trigger, uniformly.

**Not changed:** no new guardrail categories, no new telemetry types, no new refusal rows on `prescription-maps`. This section fills the existing Input validation and Refusals rows that already reference it.
